...
function Save(){
	...
	$GameTypeData = GameTypeData::getInstance();
	$exist = $GameTypeData->CheckExistName($name,$id);
	if($exist==0){
		if($id>0){
			$data_update = $GameTypeData->Update($id,$name,$name_ko_dau,$active,$sort);
			if($data_update['updatedExisting'] && $data_update['n']>0){
				$jResponse = [
					'error' => 0,
					'message' =>'ok'
				];
				$redis->del(category_game);
			}
			else{
				$jResponse = [
					'error' => 81,
			...
			$data_insert = $GameTypeData->Insert($name,$name_ko_dau,$active,$sort);
...
function Delete(){
	...
	if($id>0){
		$GameTypeData = GameTypeData::getInstance();
		$data_delete = $GameTypeData->Delete($id);
		if($data_delete['n']>0){
			$jResponse = [
				'error' => 0,
				'message' => 'ok'
