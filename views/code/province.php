...
<section class="content-header">
	<h1>
		Danh sách mã thi cấp tỉnh/thành phố
		<small></small>
	</h1>
</section>
...
<!--											<th class="col-xs-1">STT</th>-->
										...
										<tbody>
										<tr ng-repeat="province in provinces track by $index">
<!--											<td class="text-center">{{$index+1}}</td>-->
											<td class="text-center">{{province._id}}</td>
											<td>{{province.name}}</td>
											<td>{{showCode(province.codes)}}</td>
											<td class="text-center">
												<a href="javascript:void(0);" ng-click="ViewCode(province)">Chi tiết</a>
											</td>
										</tr>
										</tbody>
									</table>
			...
			<div class="modal-content">
				<form id="form" role="form" class="form-horizontal" data-toggle="validator">
					<div class="modal-header">
						...
						<h4 class="modal-title text-info"><strong>Thông tin mã thi {{province_info.name}}</strong></h4>
					</div>
								...
								<div class="row">
									<div class="col-sm-12">
										<table class="table table-bordered table-hover dataTable">
											...
											<tbody>
											<tr role="row" ng-repeat="code_info in province_info.codes track by $index">
					...
					<div class="modal-footer">
						<div class="row">
							...
							<div class="col-sm-4 button">
								<button type="submit" ng-click="AddCode(province_info._id)" class="btn btn-primary btn-save">Add</button>
				...
				<form id="form" role="form" class="form-horizontal" data-toggle="validator">
					...
					<div class="modal-footer">
						<div class="row">
							...
							<div class="col-sm-4 button">
								<button type="submit" ng-click="Save(province_info._id)" class="btn btn-primary btn-save">Save</button>
								<button type="button" class="btn btn-primary" data-dismiss="modal">Close</button>
							</div>
						</div>
					</div>
...
<script>
	var list_province = <?php echo json_encode($list_province);?>;
	$(function(){
	...
	app.controller("mainController", function($scope, $http) {
		...
		var LoadDataCode = function(province_id, callback){
			$http({
				method: "post",
				url: '/ajax/example_code_cmd.php',
				//headers: {'Content-Type': 'application/json'},
				headers: {'Content-Type': 'application/x-www-form-urlencoded'},
				data: $.param({action: 'list', type: 'province', province_id: province_id})
			})
				.then(function(res) {
					var data = res.data;
					if(data){
		...
		$scope.ViewCode = function(province_info){
			$scope.province_info = province_info;
			modal_detail.modal('show');
		};

		$scope.AddCode = function(province_id){
			$scope.code_info = {
				_id: '',
				class_id: '0',
				...
				.then(function(res) {
					...
					if(data){
						if(data.error==0){
							...
							LoadDataCode($scope.province_info._id, function(data_code){
								if(data_code.error==0){
									var content_code = data_code.content;
									$scope.province_info.codes = content_code;
								}
								else{
		...
		LoadDataCode(null,function(data_code){
			if(data_code.error==0){
				var content_code = data_code.content;
				if(content_code && content_code.length>0){
					for(var i=0, province_info; province_info = list_province[i]; i++){
						for(var j = 0, code_info; code_info = content_code[j]; j++){
							if(province_info._id == code_info.province_id){
								var codes = province_info.codes;
								if(!codes) codes = [];
								codes.push(code_info);
								province_info.codes = codes;
							}
						}
					}
				}
				$scope.provinces = list_province;
			}
			else{
				$scope.error = data.message;
			}
