			...
			<div class="row">
				...
				<div class="col-sm-5">
					<div class="form-group">
						...
						<div class="col-sm-8">
							<select class="form-control" ng-model="district_id" style="font-weight: bold;">
								<option ng-repeat="district in districts" value="{{district._id}}">{{district.name}}</option>
							</select>
						</div>
					</div>
				</div>
...
<!--											<td class="text-center">{{$index+1}}</td>-->
											<td class="text-center">{{school._id}}</td>
											<td>{{school.name}}</td>
											<td>{{showCode(school.codes)}}</td>
											<td class="text-center">
												<a href="javascript:void(0);" ng-click="ViewCode(school._id, school.name)">Chi tiết</a>
											</td>
										</tr>
										</tbody>
									</table>
			...
			<div class="modal-content">
				<form id="form" role="form" class="form-horizontal" data-toggle="validator">
					<div class="modal-header">
						...
						<h4 class="modal-title text-info"><strong>Thông tin mã thi trường {{school_info.name}}</strong></h4>
					</div>
								...
								<div class="row">
									<div class="col-sm-12">
										<table class="table table-bordered table-hover dataTable">
											...
											<tbody>
											<tr role="row" ng-repeat="code_info in school_info.codes track by $index">
												...
												<td class="text-center">
													<i ng-show="code_info.active" class="fa fa-check-square-o text-green"></i>
													<i ng-show="!code_info.active" class="fa fa-square-o text-red"></i>
												</td>
					...
					<div class="modal-footer">
						<div class="row">
							...
							<div class="col-sm-4 button">
								<button type="submit" ng-click="AddCode(school_info._id)" class="btn btn-primary btn-save">Add</button>
				...
				<form id="form" role="form" class="form-horizontal" data-toggle="validator">
					...
					<div class="modal-footer">
						<div class="row">
							...
							<div class="col-sm-4 button">
								<button type="submit" ng-click="Save(school_info._id)" class="btn btn-primary btn-save">Save</button>
								<button type="button" class="btn btn-primary" data-dismiss="modal">Close</button>
							</div>
						</div>
					</div>
...
<script>
	...
	app.controller("mainController", function($scope, $http) {
		...
		var LoadDataCode = function(province_id, district_id, school_id, callback){
			$http({
				method: "post",
				url: '/ajax/example_code_cmd.php',
				//headers: {'Content-Type': 'application/json'},
				headers: {'Content-Type': 'application/x-www-form-urlencoded'},
				data: $.param({action: 'list', type: 'school', province_id: province_id, district_id: district_id, school_id: school_id})
			})
				.then(function(res) {
					var data = res.data;
					if(data){
		...
		var LoadListSchool = function(district_id){
			$http({
				method: "POST",
				url: "/ajax/school_cmd.php",
				headers: {'Content-Type': 'application/x-www-form-urlencoded'},
				data: $.param({action: 'list', district_id:district_id})
			})
				.then(function(res) {
					var data = res.data;
					if(data){
						if(data.error==0){
							if(data.content && data.content.length>0){
								var list_school = data.content;
								LoadDataCode($scope.province_id, $scope.district_id, null, function(data_code){
									if(data_code.error==0){
										var content_code = data_code.content;
										if(content_code && content_code.length>0){
											for(var i=0, school_info; school_info = list_school[i]; i++){
												for(var j = 0, code_info; code_info = content_code[j]; j++){
													if(school_info._id == code_info.school_id){
														var codes = school_info.codes;
														if(!codes) codes = [];
														codes.push(code_info);
														school_info.codes = codes;
													}
												}
											}
										}
										$scope.schools = list_school;
									}
									else{
										$scope.error = data.message;
									}
		...
		$scope.ViewCode = function(id){
			var info = getInfoFromList(id, $scope.schools);
			if(info){
				$scope.school_info = info;
		...
		$scope.$watch('district_id', function (id) {
			if(id){
				LoadListSchool(id);
			}
			else{
		...
		$scope.AddCode = function(school_id){
			$scope.code_info = {
				_id: '',
				class_id: '0',
				school_id: school_id,
				...
				.then(function(res) {
					...
					if(data){
						if(data.error==0){
							...
							LoadDataCode($scope.province_id, $scope.district_id, $scope.code_info.school_id, function(data_code){
								if(data_code.error==0){
									var content_code = data_code.content;
									$scope.school_info.codes = content_code;
								}
								else{
									$scope.error = data.message;
								}
