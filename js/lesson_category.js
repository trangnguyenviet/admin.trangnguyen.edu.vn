...
function LoadData(){
	...
	$.ajax({
		url: "/ajax/lesson_type_cmd.php",
		type:"POST",
		dataType: "json",
		data:{action:'list',class_id:class_id},
...
function Fill_Popup(obj){
	if(obj){
		modal_title.text('Sửa thông tin chuyên đề');
		tb_id.val(obj._id);
		...
		modal_title.text('Thêm mới thông tin chuyên đề');
		tb_id.val('0');
		tb_name.val('');
		tb_rewrite.val('');
		tb_sort.val('0');
...
function Save_Data(){
	...
	$.ajax({
		url: "/ajax/lesson_type_cmd.php",
		dataType: "json",
		data:{action:'save',id:id,name:name,name_ko_dau:name_ko_dau,class_id:class_id,sort:sort},
		beforeSend: function( xhr ) {
			spinner.show();
