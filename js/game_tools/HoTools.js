/**
 * Created by tanmv on 27/12/2016.
 */
function HoTools(div,min_item,finder,callback){
	var self = this;
	this.div = div;
	this._count=0;
...
HoTools.prototype.Init = function(callback){
...
HoTools.prototype.SetEvent = function(callback){
	var self = this;
...
HoTools.prototype.Count = function(){
	return this._count;
};

HoTools.prototype.Validate = function(){
	var self = this;
	var result = {};
	var msg = '';
	...
	if(length<this._min_item){
		...
		for(var i=0;i<length;i++){
			...
			if(item_length>0){
				...
				msg +='Chưa có nội dung câu hỏi, câu: ' + (i + 1);
				// tb_answer.focus();
				break;
			}
			if(bBreak) break;
...
HoTools.prototype.SetData = function(datas,callback){
	//console.log(datas);
	//return;
	var self = this;
...
HoTools.prototype.GetData = function(){
	var result = {};
	var list_questions = this.controls.questions.find('div.question');
	var length = list_questions.length;
	if(length>0){
...
HoTools.prototype.AddItem = function(item_data,callback){
	var self = this;
	var item = $('<div class="panel panel-info question"><div class="panel-body"></div><div class="panel-footer"><div class="row text-center"><div class="col-sm-offset-4 col-sm-2"><button type="button" tn-data-index="0" class="btn btn-success form-control btn-add">Thêm phần tử</button></div><div class="col-sm-2"><button type="button" tn-data-index="0" class="btn btn-danger form-control btn-delete">Xóa câu hỏi</button></div></div></div></div>');
	...
	function AddChildItem(data){
		...
		var question_item = $('<div class="form-group question-item"> <div class="col-sm-10"> <div class="input-group"> <div class="input-group-addon"> <select class="select-type"> <option value="text">Text</option> <option value="image">Image</option> <option value="latex">Latex</option> </select> </div> <input class="form-control input-item" type="text" value="" placeholder="Giá trị"> <span class="input-group-btn"> <button class="btn btn-secondary btn-img" type="button"> <i class="fa fa-picture-o" aria-hidden="true"></i> </button> </span> </div> </div> <div class="col-sm-1"> <button type="button" class="btn btn-danger form-control btn-delete">Xóa</button> </div> </div>');
		...
		var bt_img = question_item.find('.btn-img');
		...
		function ShowEditer(ddl,input){
			...
			if('image' == type){
				...
				self.OpenLatex(input,type,input.val());
			}
			else{
...
HoTools.prototype.Destroy = function(callback){

};

HoTools.prototype.OpenLatex = function(obj,latex_type,val){
	this.latex_editer = obj;
	this.latex_type = latex_type;
	OpenLatexEditor('tb_abc','','vi-vi',false, val || '','full');
};

HoTools.prototype.SetLatex = function(latex,url,callback){
