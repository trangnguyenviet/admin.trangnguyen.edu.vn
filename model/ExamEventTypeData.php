...
class ExamEventTypeData extends ModelBase {
	protected static $collection_name = 'exam_event_types';
	...
	public function GetList(){
		$cursor = self::$collection->find([],$this::$select_field);
		$cursor->sort(['_id' => -1]);
		if($cursor->count()>0) {
			$arr = [];
	...
	public function Insert($name,$name_ko_dau,$time_begin,$time_end,$type,$active){
	...
	public function Update($id,$name,$name_ko_dau,$time_begin,$time_end,$type,$active){
		$item = [
			'name' => $name,
			'name_ko_dau' => $name_ko_dau,
			'time_begin' => $time_begin,
