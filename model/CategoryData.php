...
class CategoryData extends ModelBase{
	protected static $collection_name = 'categorys';
	...
	public static function getInstance() {
		...
		if(!self::$instance) {
			...
			// self::$collection->createIndex(['name' => 1), ['unique' => true));
			// self::$collection->createIndex(['name_ko_dau' => 1), ['unique' => true));
		}
	...
	public function GetListActive(){
		$cursor = self::$collection->find(['active'=>true,],['_id'=>1,'name'=>1,'parent_id'=>1]);
		$cursor->sort(['sort' => 1]);
		if($cursor->count()>0) {
			$arr = [];
			foreach($cursor as $item)
	...
	public function Insert($name,$parent_id,$parent_name,$name_ko_dau,$active,$sort){
	...
	public function Update($id,$name,$parent_id,$parent_name,$name_ko_dau,$active,$sort){
		$item = [
			'name' => $name,
			'parent_id' => intval($parent_id),
			'parent_name' => $parent_name,
