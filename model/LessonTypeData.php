...
class LessonTypeData extends ModelBase{
	protected static $collection_name = 'lesson_types';
	...
	public function GetListActive($class_id){
		$cursor = self::$collection->find(['class_id'=>$class_id,'active'=>true],['_id'=>1,'name'=>1]);
		$cursor->sort(['sort' => 1]);
		if($cursor->count()>0) {
			$arr = [];
			foreach($cursor as $item)
	...
	public function Insert($class_id,$name,$name_ko_dau,$active,$sort){
